...
add_filter('woocommerce_add_to_cart_validation', 'limit_cart_based_on_custom_field_value', 10, 3);

function limit_cart_based_on_custom_field_value($passed, $product_id, $quantity) {
    $product_to_add = wc_get_product($product_id);
    $new_product_value = $product_to_add->get_meta('_custom_product_text_field');

    if (!empty($new_product_value)) {
        foreach (WC()->cart->get_cart() as $cart_item) {
            if ($cart_item['product_id'] != $product_id) {
                wc_add_notice(__('Only one product can be purchased at a time.'), 'error');
                return false;
            }
        }
        ...
        foreach (WC()->cart->get_cart() as $cart_item) {
            $existing_product = wc_get_product($cart_item['product_id']);
            $existing_value = $existing_product->get_meta('_custom_product_text_field');

            if (!empty($existing_value)) {
                wc_add_notice(__('You cannot add other products with a single-purchase product.'), 'error');
                return false;
            }
        }
    }
...
function add_custom_product_fields() {
    ...
    woocommerce_wp_text_input( array(
        'id'          => '_custom_product_single',
        'label'       => __('Single', 'woocommerce'),
        'placeholder' => 'Enter single value',
        'desc_tip'    => true,
        'description' => 'This is the Single value.',
    ) );
    
   
}

add_action('woocommerce_product_options_advanced', 'add_custom_field');
function add_custom_field() {
 woocommerce_wp_text_input( array(
        'id'          => '_custom_product_manufacturer',
        'label'       => __('Manufacturer', 'woocommerce'),
        'placeholder' => 'Enter manufacturer name',
        'desc_tip'    => true,
        'description' => 'This is the Manufacturer field.',
    ) );
...
function save_custom_product_fields($post_id) {
    if ( isset($_POST['_custom_product_single']) ) {
        update_post_meta(
            $post_id,
            '_custom_product_single',
            sanitize_text_field($_POST['_custom_product_single'])
        );
    }

    if ( isset($_POST['_custom_product_manufacturer']) ) {
        update_post_meta(
            $post_id,
            '_custom_product_manufacturer',
            sanitize_text_field($_POST['_custom_product_manufacturer'])
        );
    }
}
...
function display_custom_product_fields() {
    global $post;
    $manufacturer = get_post_meta($post->ID, '_custom_product_manufacturer', true);

    if ($manufacturer) {
        echo '<p><strong>Manufacturer:</strong> ' . esc_html($manufacturer) . '</p>';
    }
}

add_action('woocommerce_after_shop_loop_item_title', 'display_manufacturer_on_shop', 10);
function display_manufacturer_on_shop() {
    global $product;
    $manufacturer = get_post_meta($product->get_id(), '_custom_product_manufacturer', true);

    if ($manufacturer) {
        echo '<p class="manufacturer-shop"> ' . esc_html($manufacturer) . '</p>';
    }
}



add_action( 'woocommerce_single_product_summary', 'gs_product_acf_value_label', 7 );
function gs_product_acf_value_label(){
    global $product;

    $color = get_field_object( 'colors', $product->get_id() );
    $gender = get_field_object( 'gender', $product->get_id() );

    if( ! empty( $color['value'] ) ){
        echo '<p><b>'.$color['label'].'</b>: '. $color['value'] . '</p>';
    }
    if( ! empty( $gender['value'] ) ){
        echo '<p><b>'.$gender['label'].'</b>: '. $gender['value'] . '</p>';
    }
...
function bbloomer_display_fake_product_position_4() {
   ...
   if ( $loop_index === 5 ) {
      ...
      echo '<a href="https://www.instagram.com/p/DHGHJ4Cx8jT/?img_index=1" target="_blank">';
      echo '<img src="data:image/jpeg;base64,">';
...
function enqueue_custom_woocommerce_styles() {
    if (is_wc_endpoint_url('order-received')) {
        wp_enqueue_style('custom-thankyou-style', get_stylesheet_directory_uri() . '/thankyou-style.css');
    }
}
add_action('wp_enqueue_scripts', 'enqueue_custom_woocommerce_styles');


function custom_enqueue_font_awesome() {
    // Load Font Awesome from CDN
    wp_enqueue_style( 'font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css' );
}
add_action( 'wp_enqueue_scripts', 'custom_enqueue_font_awesome' );
